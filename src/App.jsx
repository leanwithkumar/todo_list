...
function App() {
  ...
  const removethis = (targetindex) => {
    setTodo((prevtodo) =>
      prevtodo.filter((_, index) => index !== targetindex)
    );
  };
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 space-y-6">
        <form onSubmit={addTodo} className="flex flex-col sm:flex-row gap-4">
          <input
            ...
            className="w-full p-3 border-2 border-gray-300 rounded-xl shadow-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition"
          />
          <button
            type="submit"
            className="w-full sm:w-auto px-6 py-3 bg-purple-600 text-white rounded-xl shadow-md hover:bg-purple-700 transition transform hover:scale-105"
          >
            Add Todo
          </button>
        </form>
